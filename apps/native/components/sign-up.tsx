...
export function SignUp() {
	...
	const handleSignUp = async () => {
		...
		await authClient.signUp.email(
			...
			{
				onError: (error) => {
					setError(error.error?.message || t('auth.signUpFailed'));
					setIsLoading(false);
				},
				onSuccess: () => {
					setName("");
			...
			>
				...
				>
					{isLoading ? (
						...
						<Button.Label className="text-black">{t('auth.signUp')}</Button.Label>
					)}
				</Button>
				...
				<View className="flex-row items-center justify-center mb-4">
					<Pressable onPress={() => router.push("/login")}>
						<Text className="text-sm text-muted">
							...
							<Text className="text-accent font-medium">{t('auth.signIn')}</Text>
						</Text>
					</Pressable>
				</View>
				...
				<View className="flex-row items-center justify-center">
					...
					<Pressable onPress={() => {}}>
						<Text className="text-sm text-muted">{t('auth.helpCenter')}</Text>
					</Pressable>
				</View>
			</AnimatedView>
		</View>
